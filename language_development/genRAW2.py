...
#
in_path_par = "C:/P4/Users/jdenys/text_input_data/en/try/"  # input directory with text files
out_path_par = "C:/tmp/"                                # output directory to write the RAW file
language_par = "nl"                                     # language selector
...
def get_literal(text_string, index_start, index_stop) -> str:
    ...
    print(correct_index(index_start),":",correct_index(index_stop))
    print(index_start-correct_index(index_start),index_stop-correct_index(index_stop))
    ...
    for sent in engine.m_index['sentences']: # S志望学部の決定時期について経営学部生に関しては表7 （ a ） （ b ）に、経済学部生に関しては表8 （ a ） （ b ）に示す。
        ...
        if OldStyle:
            ...
            for entity in sent['entities']:
                ...
                lit_text = get_literal(text, entity['offset_start'], entity['offset_stop'])
                print(']',lit_text,'[')
