...
bool iKnow_KB_Rule::ImportFromCSV(string rules_csv, CSV_DataGenerator& kb)
{
	...
	if (ifs.is_open()) {
		...
		for (string line; getline(ifs, line);)
		{
			...
			// rule.ProcessRuleOutputPattern(row_rule[4 - 1], rule.Phase, kb, auto_generated_labels); // collect phases for auto-generated S-labels
...
std::string iKnow_KB_Rule::TransformRulePattern(string& pattern, string& phase, CSV_DataGenerator& kb, newLabels_type &newLabels, newLabelsIndex_type &newLabelsIndex, AutoGeneratedLabels_type &AutoGeneratedLabels)
{
	// Set len = $L(pattern, "|")
	vector<string> labels = kb.split_row(pattern, '|');
