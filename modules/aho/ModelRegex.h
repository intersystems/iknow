...
namespace iknow {
  namespace model {
    ...
    class Regex {
    public:
      //TODO: Ownership/Cleanup code now that these objects can be created dynamically,
      //repeatedly by dynamic compiled model loading
      Regex() {}
      //We can't throw an exception while these are being built statically, so
      //we keep the error state around for checking on use.
      //TODO: Check size when truncating to 32-bit
      Regex(const String& regex) :
	error_code_(U_ZERO_ERROR),
	pattern_(icu::RegexPattern::compile(icu::UnicodeString(regex.data(), static_cast<int32_t>(regex.size())),
					    parse_error_, error_code_)),
	matcher_(pattern_->matcher(error_code_)) /* , regex_text_(regex) */ {
	...
	*/
	//This is a read-only "intrusive" constructor: The UnicodeString will use the passed
	//buffer and not free it when destroyed.
